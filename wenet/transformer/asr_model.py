...
class ASRModel(torch.nn.Module):
    ...
    def forward(
        ...
        # 1. Encoder
        transpose_prob = torch.rand(1).item()
        encoder_out, encoder_mask = self.encoder(speech, speech_lengths)
        # encoder_out_l, encoder_mask_l = self.encoder(speech, speech_lengths)
        ...
        if self.ctc_weight != 0.0:
            loss_ctc, ys_hat = self.ctc(encoder_out, encoder_out_lens, text, text_lengths)

        else:
            loss_ctc = None
            ys_hat = None

        if 'BAC009S0002W0123' in key and ys_hat is not None:
            idx = key.index('BAC009S0002W0123')
            w0122 = ys_hat[:, idx, :]
            log_str = " ".join(["%.4f" % x for x in w0122[:, 0]])
            logging.info('BAC009S0002W0123: ' + str(encoder_out_lens[idx].item()) + ", " + str(transpose_prob > 0.5) + ", " + log_str)
